...
function ConnectionErrorDisplay() {
  return (
...
function ConnectionWatcher({ children }: { children: ReactNode }) {
    ...
    <LiveblocksErrorBoundary>
      {hasSecretKey ? (
        <LiveblocksProvider authEndpoint="/api/liveblocks-auth">
          <RoomProviderContent roomId={roomId} fallback={fallback}>
            {children}
          </RoomProviderContent>
        </LiveblocksProvider>
      ) : (
...
function RoomProviderContent({ roomId, children, fallback }: { roomId: string; children: ReactNode; fallback?: ReactNode }) {
  return (
    <RoomProvider
      id={roomId}
      initialPresence={{
    ...
    >
      <ClientSideSuspense fallback={fallback || <LoadingFallback />}>
        {() => <ConnectionWatcher>{children}</ConnectionWatcher>}
      </ClientSideSuspense>
    </RoomProvider>
  );
}
